...
interface GetFoodNameStrategy {
  getFoodName: (key: string) => string;
}

class PizzaNameStrategy implements GetFoodNameStrategy {
  getFoodName(key: string): string {
    switch (key) {
      case 'a':
        return '핫마요피자(M)';
...
class ChickenNameStrategy implements GetFoodNameStrategy {
  getFoodName(key: string): string {
    switch (key) {
      case 'a':
        return '모리치킨(2마리)';
...
export default class FoodPopup implements Popup {
  private getFoodNameStrategy: GetFoodNameStrategy;
  constructor(menu: 'PIZZA' | 'CHICKEN') {
    this.getFoodNameStrategy =
      menu === 'PIZZA' ? new PizzaNameStrategy() : new ChickenNameStrategy();
  ...
  sendSelectedList() {
    ...
    const translate = (itemEl: NodeListOf<HTMLInputElement>) =>
      Array.from(itemEl).reduce<string[]>((acc, el) => {
        ...
        acc.push(this.getFoodNameStrategy.getFoodName(menuId));
